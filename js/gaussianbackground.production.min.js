 ...
 * @twitter    @furiousfoxx
 */ 
function GaussianBackground(d,c,a){if(!(this instanceof GaussianBackground))return new GaussianBackground(d,c,a);this.animationFrame=this.context=null;this.timestep=0;this.lastCallTime=Date.now();this.layers={};this.layersBackup={};this.options={blur:!0,blurRadius:50,blurMethod:"stackblur",blurIterations:1,animation:!0,fpsCap:20,renderWidth:320,renderHeight:130};if(!(this.context=d.getContext("2d")))return console.log("ERROR: Could not load canvas"),!1;this.updateOptions(a);this.updateLayers(c);this.context.canvas.width=
this.options.renderWidth;this.context.canvas.height=this.options.renderHeight;this.animationFrame=window.requestAnimationFrame(this.displayLoop.bind(this))}
