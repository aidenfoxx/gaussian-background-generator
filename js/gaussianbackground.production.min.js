...
GaussianBackground.prototype.drawBackground=function(){for(var d=Object.keys(this.layers).length-1;0<=d;d--){var c=this.layers[d].context,a=this.layers[d].orbs;c.fillStyle=this.layers[d].color;c.fillRect(0,0,this.options.renderWidth,this.options.renderHeight);for(var b=Object.keys(a).length-1;0<=b;b--)this.options.animation&&(a[b].posX+=a[b].velX,a[b].posY+=a[b].velY,a[b].posX>=a[b].maxX?(a[b].posX=a[b].maxX,a[b].velX=-a[b].velX):a[b].posX<=a[b].minX&&(a[b].posX=a[b].minX,a[b].velX=-a[b].velX),a[b].posY>=
a[b].maxY?(a[b].posY=a[b].maxY,a[b].velY=-a[b].velY):a[b].posY<=a[b].minY&&(a[b].posY=a[b].minY,a[b].velY=-a[b].velY)),c.save(),c.globalCompositeOperation="destination-out",c.beginPath(),c.arc(a[b].posX,a[b].posY,a[b].radius,0,2*Math.PI,!1),c.fill(),c.restore();this.context.drawImage(c.canvas,0,0)}};
