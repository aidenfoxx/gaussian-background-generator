...
GaussianBackground.prototype.drawBackground=function(){for(var c=Object.keys(this.layers).length-1;0<=c;c--){var d=this.layers[c].context,b=this.layers[c].orbs;d.fillStyle=this.layers[c].color;d.fillRect(0,0,this.options.renderWidth,this.options.renderHeight);for(var a=Object.keys(b).length-1;0<=a;a--)b[a].posX+=b[a].velX,b[a].posY+=b[a].velY,b[a].posX>=b[a].maxX?(b[a].posX=b[a].maxX,b[a].velX=-b[a].velX):b[a].posX<=b[a].minX&&(b[a].posX=b[a].minX,b[a].velX=-b[a].velX),b[a].posY>=b[a].maxY?(b[a].posY=
b[a].maxY,b[a].velY=-b[a].velY):b[a].posY<=b[a].minY&&(b[a].posY=b[a].minY,b[a].velY=-b[a].velY),d.save(),d.globalCompositeOperation="destination-out",d.beginPath(),d.arc(b[a].posX,b[a].posY,b[a].radius,0,2*Math.PI,!1),d.fill(),d.restore();this.context.drawImage(d.canvas,0,0)}};
